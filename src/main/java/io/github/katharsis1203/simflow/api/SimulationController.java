...
public class SimulationController {
    ...
    @GetMapping
    public List<SimulationRunResponse> getSimulationRuns(){
        return runRepository.findAll()
                .stream()
                .map(run -> new SimulationRunResponse(
                        run.getId(),
                        run.getResourceName(),
                        run.getInitialAmount(),
                        run.getProductionPerHour(),
                        run.getConsumptionPerHour(),
                        run.getDurationHours(),
                        run.getFinalAmount(),
                        run.getStartedAt()
                ))
                .toList();
